...
s.textContent="
  #shrf-btn{position:fixed;bottom:90px;right:24px;width:64px;height:64px;cursor:pointer;z-index:2147483647;background:none;border:none;padding:0;filter:drop-shadow(0 6px 16px rgba(26,126,200,.3));}
  #shrf-bub{display:none;position:fixed;bottom:162px;right:12px;background:#fff;border:1.5px solid #c8e4f8;border-radius:12px;padding:6px 14px;font-size:12px;font-weight:600;color:#1A7EC8;font-family:sans-serif;box-shadow:0 3px 12px rgba(26,126,200,.2);z-index:2147483646;white-space:nowrap;transition:opacity .5s;cursor:pointer;}
  ...
  .shrf-pc:hover{border-color:#1A7EC8;box-shadow:0 4px 12px rgba(26,126,200,.15);}
  .shrf-pc img{width:100%;height:130px;object-fit:contain;background:#f0f8ff;padding:10px;display:block;box-sizing:border-box;}
  .shrf-pc-body{padding:8px 10px 10px;}
  .shrf-pc-brand{font-size:9px;color:#7ab8d8;text-transform:uppercase;letter-spacing:.5px;font-weight:600;}
...
window.shrfToggle=function(){
  ...
  if(shrfOpen&&shrfHistory.length===0){
    shrfAddMsg('Merhaba! 👋 Ben SHARAF AI, kişisel alışveriş asistanınızım. Size nasıl yardımcı olabilirim?','ai');
    shrfSetChips(['📱 Telefonlar','📺 TV Fırsatlar','📍 Mağazalar','🚚 Teslimat']);
  }
};
...
function shrfAddMsg(html,who){
  ...
  var av=who==='ai'?'<div class="shrf-av ai">AI</div>':'<div class="shrf-av u">SİZ</div>';
...
function shrfCards(prods){
  ...
  prods.forEach(function(p){
    var img=p.i?'<img src="'+p.i+'" alt="'+p.n+'" onerror="this.style.display=\'none\'"/>':'';
    h+='<a class="shrf-pc" href="'+(p.u||'#')+'" target="_blank">'+img+'<div class="shrf-pc-body"><div class="shrf-pc-brand">'+(p.b||'')+'</div><div class="shrf-pc-name">'+(p.n||'')+'</div><div class="shrf-pc-row"><span class="shrf-pc-price">'+(p.p||'')+'</span><span class="shrf-pc-btn">Ürüne Git →</span></div></div></a>';
  });
  return h;
}
...
function shrfSetChips(arr){
  ...
  if(!c)return;
  if(!arr||!arr.length)arr=['Başka ürün','Daha ucuz?','Stok?'];
...
window.shrfSend=function(){
  ...
  .then(function(data){
    ...
    }catch(e){shrfAddMsg(raw||'Bir hata oluştu.','ai');shrfSetChips(['Tekrar dene']);}
    shrfBusy=false;
  })
  .catch(function(){
    shrfHideTyping();
    shrfAddMsg('Bağlantı hatası. Lütfen tekrar deneyin.','ai');
